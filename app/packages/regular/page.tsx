...
const RegularPackagesPage = () => {
  ...
  const getPackageImage = (pkg: Package): { url: string; alt: string } | null => {
    if (!pkg.images || pkg.images.length === 0) return null;
    const firstImage = pkg.images[0] as unknown;
    ...
    if (
      ...
      return {
        ...
        alt:
          'alt' in firstImage && typeof (firstImage as { alt?: unknown }).alt === 'string'
            ? (firstImage as { alt?: string }).alt || pkg.title
            : pkg.title,
      };
    }
